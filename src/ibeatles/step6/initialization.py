...
class Initialization:
    ...
    def combobox(self):
        ...
        self.parent.ui.interpolation_comboBox.setCurrentIndex(
            DEFAULT_INTERPOLATION_INDEX
        )
    ...
    def range_slider(self):
        layout = QVBoxLayout()
        self.parent.ui.range_slider = QRangeSlider(
            splitterWidth=100, vertical=True, min_at_the_bottom=True
        )
        ...
        self.parent.ui.range_slider.setRealRange(
            real_min + 1e-10, real_max
        )  # trick to display full range #BUG
    ...
    def data(self):
        ...
        [self.parent.image_size["height"], self.parent.image_size["width"]] = np.shape(
            integrated_image
        )
    ...
    def matplotlib(self):
        ...
        self.parent.matplotlib_plot = _matplotlib(
            parent=self.parent, widget=self.parent.ui.matplotlib_widget
        )
    ...
    def min_max_values(self):
        [_, x0, y0, width, height, bin_size] = self.grand_parent.session_dict[
            DataType.bin
        ][SessionSubKeys.roi]

        max_row_index = self.grand_parent.session_dict[SessionKeys.bin][
            SessionSubKeys.nbr_row
        ]
        max_col_index = self.grand_parent.session_dict[SessionKeys.bin][
            SessionSubKeys.nbr_column
        ]

        d_array = self.parent.d_array
        d_array_roi = d_array[
            y0 : y0 + (max_row_index * bin_size), x0 : x0 + (max_col_index * bin_size)
        ]
        ...
        strain_mapping_roi = strain_mapping[
            y0 : y0 + (max_row_index * bin_size), x0 : x0 + (max_col_index * bin_size)
        ]
