...
class FittingInitializationHandler(object):
    ...
    def retrieve_parameters_and_update_table(self):
        table_handler = TableDictionaryHandler(
            parent=self.parent, grand_parent=self.grand_parent
        )
        ...
        table_handler.fill_table_with_variable(
            variable_name="d_spacing", value=d_spacing, all_keys=True
        )
        ...
        if np.isnan(sigma):
            self.all_variables_initialized is False
        table_handler.fill_table_with_variable(
            variable_name="sigma", value=sigma, all_keys=True
        )
        ...
        if np.isnan(alpha):
            self.all_variables_initialized is False
        table_handler.fill_table_with_variable(
            variable_name="alpha", value=alpha, all_keys=True
        )
        ...
        if self.advanced_mode:
            ...
            table_handler.fill_table_with_variable(
                variable_name="a2", value=a2, all_keys=True
            )
            ...
            table_handler.fill_table_with_variable(
                variable_name="a5", value=a5, all_keys=True
            )
            ...
            table_handler.fill_table_with_variable(
                variable_name="a6", value=a6, all_keys=True
            )
            ...
            table_handler.fill_table_with_variable(
                variable_name="a1", value=a1, all_keys=True
            )
    ...
    def isolate_left_and_right_part_of_inflection_point(self):
        ...
        self.selection_range["left_range"]["y_axis"] = full_y_axis[
            left_index:inflection_point_index
        ]
        self.selection_range["left_range"]["x_axis"] = full_x_axis[
            left_index:inflection_point_index
        ]
        self.selection_range["right_range"]["y_axis"] = full_y_axis[
            inflection_point_index:
        ]
        self.selection_range["right_range"]["x_axis"] = full_x_axis[
            inflection_point_index:
        ]
    ...
    def get_a1(self):
        if self.advanced_mode:
            ...
            nbr_data_to_remove = int(
                (self.percentage_of_data_to_remove_on_side / 100.0) * nbr_data
            )
    ...
    def get_d_spacing(self):
        ...
        """
        print(
            f"self.parent.ui.lambda_min_lineEdit.text(): {self.parent.ui.lambda_min_lineEdit.text()}"
        )
